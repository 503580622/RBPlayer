...
static NSArray *RBPlayerTimeSliderObservedKeyPaths() {
    static NSArray *_RBPlayerTimeSliderObservedKeyPaths = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _RBPlayerTimeSliderObservedKeyPaths = @[NSStringFromSelector(@selector(maximumTrackTintColor)),
    ...
    return _RBPlayerTimeSliderObservedKeyPaths;
}

static void *RBPlayerTimeSliderObserverContext = &RBPlayerTimeSliderObserverContext;

@interface RBPlayerSlider ()
    ...
    if (self) {
        ...
        for (NSString *keyPath in RBPlayerTimeSliderObservedKeyPaths()) {
            if ([self respondsToSelector:NSSelectorFromString(keyPath)]) {
                [self addObserver:self forKeyPath:keyPath options:NSKeyValueObservingOptionNew context:RBPlayerTimeSliderObserverContext];
            }
        }
    }
    return self;
...
- (void)dealloc {
    for (NSString *keyPath in RBPlayerTimeSliderObservedKeyPaths()) {
        if ([self respondsToSelector:NSSelectorFromString(keyPath)]) {
            [self removeObserver:self forKeyPath:keyPath context:RBPlayerTimeSliderObserverContext];
        }
    }
}
...
{
    if (context == RBPlayerTimeSliderObserverContext) {
